...
async def predict_spans(inp: PredictIn):
    ...
    try:
        ...
        results = []
        for s in spans:
            out = await run_in_threadpool(lambda t=s["text"]: classifier(t)[0])
        ...
        overall = await run_in_threadpool(lambda: classifier(text)[0])
        ...
        return {
            "overall": {"label": overall_label, "confidence": overall_conf},
            "spans": results,
            "meta": {
                "latency_ms": latency_ms,
                "http_in_flight": snap.http_in_flight,
